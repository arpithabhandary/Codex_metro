...
export const CONTRACTOR_MESSAGES_STORAGE_KEY = "kai-contractor-messages";
...
export async function approveWorkRecord(record: WorkRecord, allRecords: WorkRecord[], contractorDetails: { wage: number; hoursWorked: number; location: string; date: string; description: string }): Promise<WorkRecord> {
  const timestamp = new Date().toISOString();
  const detailsMatched = contractorDetails.wage === record.wage && contractorDetails.hoursWorked === record.hoursWorked && contractorDetails.location.trim().toLowerCase() === record.location.trim().toLowerCase() && contractorDetails.date === record.date && contractorDetails.description.trim().toLowerCase() === record.description.trim().toLowerCase();
  if (!detailsMatched) return { ...record, status: "Needs Revision", verificationStatus: "Pending review", verificationMetadata: { ...record.verificationMetadata, contractorName: record.contractorName, contractorSubmittedDetails: { ...contractorDetails, submittedAt: timestamp }, detailsMatched: false, history: [...(record.verificationMetadata?.history || []), { action: "Requested changes", timestamp, contractorName: record.contractorName, reason: "Contractor-submitted work details did not match the worker record." }] } };
  ...
  return { ...record, status, verificationStatus: verification.verificationStatus, verification, verificationMetadata: { ...record.verificationMetadata, verificationId, verificationTimestamp: timestamp, contractorName: record.contractorName, contractorCompany: record.contractorName, contractorSubmittedDetails: { ...contractorDetails, submittedAt: timestamp }, detailsMatched: true, history: [...(record.verificationMetadata?.history || []), { action: "Approved" as const, timestamp, contractorName: record.contractorName, reason: status === "Needs Revision" ? "AI fraud check flagged this record for manual review." : undefined }] } };
}
